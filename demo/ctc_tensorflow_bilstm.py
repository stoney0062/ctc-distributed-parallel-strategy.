...
with tf.Session(graph=graph) as session:
    ...
    saver.save(session,'./bilstm/lstm-model.ckpt')
    ...
    d0 = session.run(decoded[0], feed_dict=feed)
    #d1 = session.run(decoded[1], feed_dict=feed)
    #d2 = session.run(decoded[2], feed_dict=feed)
    d0 = d_beam[0]
    #d1 = d_beam[1]
    #d2 = d_beam[2]
    dense_decoded0 = tf.sparse_tensor_to_dense(d0, default_value=-1).eval(session=session)
    #dense_decoded1 = tf.sparse_tensor_to_dense(d1, default_value=-1).eval(session=session)
    #dense_decoded2 = tf.sparse_tensor_to_dense(d2, default_value=-1).eval(session=session)
    dictnory = {} 
