...
def _tower_loss( inputs, targets ,seq_len , istrain ):
		"""
            ...
            with tf.variable_scope("tower"):
                ...
                for i in xrange( 4 ):
                    with tf.device('/gpu:%d' % i):
                        ...
                        loss,ler = _tower_loss( x_splits[i], y_splits[i] , seq_len_split[0] , istrain )
...
with tf.Session(graph=graph,config=config) as session:
    ...
    d_beam = session.run(decode, feed_dict=feed)
    print d_beam
    d0 = d_beam[0]
